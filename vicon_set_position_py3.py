...
def msg_to_se3(msg):
    ...
    if np.abs(norm - 1.0) > 1e-3:
        ...
        q = q / norm
    # NOTE: below 4 lines should give the homogenous rotation matrix from quaternions (python 3), instead of the tr.quaternion_matrix(q) line (python 2)
...
class ViconPositionSetNode:
    ...
    def set_position_local_cb(self, msg):
        ...
        T_D_V_euler_angles = Rotation.from_matrix(T_D_V[0:3,0:3]).as_euler("xyz")
        print("VICON euler angles (VICON to Drone)", T_D_V_euler_angles)
        ...
        T_R_D = np.eye(4)
        T_R_D_euler_angles = Rotation.from_matrix(T_D_V[0:3,0:3]).as_euler("xyz")
        print("Realsense euler angles (Drone to Realsense)", T_R_D_euler_angles)
        ...
        T_R_V_euler_angles = Rotation.from_matrix(T_D_V[0:3,0:3]).as_euler("xyz") 
        print("VICON to realsense euler angles", T_R_V_euler_angles)
        ...
        p = T_R_D[0:3, -1]
        R_R_D = T_R_D[0:3,0:3]
        q = Rotation.from_matrix(R_R_D).as_quat()
